...
JSX = syntactic sugar, it *compiles* to JS. 
...
Need a tool to run the compile process - *Babel*, for example

Interpolation - insertion of something of a different nature into something  else. 
Template literals are an example of interpolation.

- The reason statements (like if statements) can't be allowed in curly braces is bc it's 
like trying to pass an if statement as a parameter to the React.createElement() method - doesn't
happen. 

When spreading objects, whatever property comes later will override the same property if set earlier
## Background
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=03%3A%20Using%20JSX&em=kimkvn%40gmail.com).
