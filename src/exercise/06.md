...
event.target.preventDefault() prevents default behavior, in the case of forms,
means that prevents the form/page from refreshing (default behavior)

to access the target val of the input, use `event.target.elements[index].value`
or by input name or id `event.target.elements.nameOrId.value`

In react when working with `event`, we actually work with `SyntheticEvent` but
we interact with it the same way as the native window `event`. React does this
for performance reasons

`htmlFor` / labeling your inputs is required for accessibility / screen readers

A `ref` stays consistent inbetween re-renders of a component Refs are mutable
and can hold a value in its `.current` property Handy for keeping track of a
value inbetween renders??? (what)

`ref`s can be used to access nodes instead of `event` as an option

Refs TO BE CONTINUED...

Programmatically controlled inputs are kinda cool Wonder about the ramifications
of "controlling" user behavior though
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=kimkvn%40gmail.com).
