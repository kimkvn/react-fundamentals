...
Can use destructuring when making custom functional components

```
  func(props) => <div>{props.children}</div>
```

can be: 

```
  func({children}) => <div>{children}</div>
```

You can pass a function into the first argument of React.createElement()

Production build of React does not run PropTypes - very costly in performance

- Fragments = `<> </>` allows you to render children components without additional node elements to the dom 
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=kimkvn%40gmail.com).
