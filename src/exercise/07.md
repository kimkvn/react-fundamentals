...
On the `key` prop: If I want to update/re-render a mapped list of items with an
added item, React doesn't automatically know if it needs to update the
beginning, middle, or end of the list. Same with removing an item. `key` keeps
track of list items so it doesn't mix things up

Setting `key` to a list item `index` isn't much of an improvement. While it
fixes the console warning, React still won't have a clear understanding if you
start manipulating things in the beginning or middle of the list
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=07%3A%20Rendering%20Arrays&em=kimkvn%40gmail.com).
